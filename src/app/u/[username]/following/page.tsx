import Link from "next/link";
import { notFound } from "next/navigation";
import { getCurrentUser } from "@/lib/auth";
import { getFollowCounts, getFollowing, getFollowingSet, getUserByUsername } from "@/lib/db/queries";
import UserList from "@/components/UserList";

export default async function FollowingPage({
  params,
}: {
  params: Promise<{ username: string }>;
}) {
  const { username } = await params;
  const [profileUser, currentUser] = await Promise.all([
    getUserByUsername(username),
    getCurrentUser(),
  ]);
  if (!profileUser) notFound();

  const [following, counts] = await Promise.all([
    getFollowing(profileUser.id),
    getFollowCounts(profileUser.id),
  ]);

  const followingIds = currentUser
    ? await getFollowingSet(
        currentUser.id,
        following.map((u) => u.id)
      )
    : new Set<string>();

  return (
    <div className="flex flex-col gap-6">
      <div>
        <Link href={`/u/${username}`} className="text-sm text-[var(--accent)]">
          ← {profileUser.displayName}
        </Link>
        <h1 className="text-xl font-bold mt-1">Following</h1>
      </div>

      <div className="flex items-center gap-4 border-b border-[var(--border)] text-sm">
        <Link href={`/u/${username}/followers`} className="pb-2 text-[var(--muted)]">
          Followers ({counts.followers})
        </Link>
        <span className="pb-2 border-b-2 border-[var(--accent)] font-medium">
          Following ({counts.following})
        </span>
      </div>

      <UserList
        users={following}
        currentUserId={currentUser?.id}
        followingIds={followingIds}
        redirectPath={`/u/${username}/following`}
        emptyMessage={`${profileUser.displayName} isn't following anyone yet.`}
      />
    </div>
  );
}
